const DAMAGE_PART_ORDER = ['nose', 'hull', 'leftWing', 'rightWing', 'leftEngine', 'rightEngine', 'tail', 'gear'];

ensureHudAdditions = function ensureHudAdditionsOverride() {
  if (!document.getElementById('cursorFlightStyles')) {
    const style = document.createElement('style');
    style.id = 'cursorFlightStyles';
    style.textContent = `
      .pilot-stack strong { color: rgba(221, 252, 255, 0.96); }
      .segment.pilot-mode button.active {
        color: #061317;
        background: linear-gradient(135deg, rgba(151, 250, 255, 0.96), rgba(220, 255, 244, 0.88));
        box-shadow: 0 0 24px rgba(102, 235, 255, 0.3);
      }
      .cursor-reticle {
        position: fixed;
        left: 50%;
      ...
      .cursor-reticle::after {
        ...
        inset: 0;
        border-radius: 50%;
        border: 1px solid rgba(159, 247, 255, 0.78);
        box-shadow: 0 0 18px rgba(84, 226, 255, 0.32), inset 0 0 18px rgba(84, 226, 255, 0.16);
      }
      .cursor-reticle::after {
        inset: 18px;
      ...
      .cursor-reticle span {
        ...
        height: 1px;
        transform: translate(-50%, -50%);
        background: linear-gradient(90deg, transparent, rgba(175, 247, 255, 0.7), transparent);
      }
      .cursor-reticle span + span { transform: translate(-50%, -50%) rotate(90deg); }
      .damage-vignette.active {
        background:
          radial-gradient(ellipse at center, transparent 42%, rgba(255, 22, 46, calc(var(--damage-alpha) * 0.82)) 100%),
          linear-gradient(90deg, rgba(255, 37, 55, calc(var(--damage-alpha) * 0.72)), transparent 20%, transparent 80%, rgba(255, 37, 55, calc(var(--damage-alpha) * 0.72))),
          linear-gradient(0deg, rgba(255, 37, 55, calc(var(--damage-alpha) * 0.58)), transparent 18%, transparent 78%, rgba(255, 37, 55, calc(var(--damage-alpha) * 0.42)));
      }
      .damage-vignette.critical::before {
        content: '';
        position: absolute;
        inset: 0;
        background: repeating-linear-gradient(135deg, rgba(255, 52, 61, 0.12) 0 12px, transparent 12px 30px);
        opacity: 0.65;
        animation: warningScan 0.9s linear infinite;
      }
      .damage-part[data-severity='medium'] {
        border-color: rgba(255, 171, 64, 0.96);
        background: rgba(255, 128, 36, 0.36);
        box-shadow: 0 0 18px rgba(255, 146, 39, 0.68), inset 0 0 14px rgba(255, 255, 255, 0.18);
      }
      .damage-part[data-severity='critical'] {
        border-color: rgba(255, 55, 79, 1);
        background: rgba(255, 15, 46, 0.58);
        box-shadow: 0 0 24px rgba(255, 35, 55, 0.86), inset 0 0 18px rgba(255, 255, 255, 0.24);
      }
      .damage-list li span { display: grid; gap: 1px; }
      .damage-list li em {
        font-style: normal;
        font-size: 10px;
        color: rgba(255, 218, 222, 0.62);
      }
      @keyframes cursorPulse {
        0%, 100% { transform: translate(-50%, -50%) scale(0.88); opacity: 0.54; }
      ...
      @media (max-width: 760px) {
        .cursor-reticle { width: 44px; height: 44px; }
      }
    `;
  ...
  if (systems && !document.getElementById('pilotModeSegment')) {
    ...
    value.id = 'pilotModeValue';
    value.textContent = 'Cursor';
    ...
    ['cursor', 'keyboard'].forEach((mode) => {
      const button = document.createElement('button');
      button.type = 'button';
      button.dataset.pilotMode = mode;
      button.textContent = mode === 'cursor' ? 'Cursor' : 'Keyboard';
...
launch = function launchOverride() {
  ...
  ui.message.textContent = pilotMode === 'cursor'
    ? 'Cursor flight live. Point where you want the jet to go; throttle and gear stay on the keyboard.'
...
resetAircraft = function resetAircraftOverride(first) {
  ...
  updateDamageUi();
  setPilotMode(pilotMode, true);
  if (!first) ui.message.textContent = 'New airframe spawned higher and stabilized for another try.';
};
...
createLightningBolt = function createLightningBoltOverride(start, end, hitPlane, targetPart) {
  ...
  for (let i = 0; i < branchCount; i++) {
    ...
    const branch = buildJaggedPoints(anchor, anchor.clone().add(dir), 5, hitPlane ? 34 : 72, i * 41.2 + end.z);
    addBoltLine(group, branch, i % 2 ? 0xd6ffff : 0x74dfff, 0.62, 10 + i);
  }

  if (hitPlane) {
    const shockMat = new THREE.MeshBasicMaterial({ color: 0xbdfdff, transparent: true, opacity: 0.68, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide });
    const shock = new THREE.Mesh(new THREE.RingGeometry(24, 58, 64), shockMat);
...
updateDamageUi = function updateDamageUiOverride(reason) {
  ...
  if (ui.damageList) {
    if (damaged.length === 0) {
      ui.damageList.innerHTML = '<li><span>all systems<em>green</em></span><strong>0%</strong></li>';
    } else {
  ...
  for (let i = fireGroups.length - 1; i >= 0; i--) {
    ...
    item.children.forEach((child, childIndex) => {
      if (child.isMesh) {
        ...
        if (child.userData.kind === 'flame') {
          child.material.opacity = 0.68 + Math.sin(t * 7.0 + childIndex) * 0.12;
        }
      }
...
function cursorGroundPoint() {
  ...
  if (cursorRaycaster.ray.intersectPlane(cursorGroundPlane, cursorGround)) return cursorGround;
  return null;
}
...
function createExplosionMarker(item) {
  ...
  const scorch = new THREE.Mesh(
    new THREE.CircleGeometry(125, 64),
    new THREE.MeshBasicMaterial({ color: 0x050303, transparent: true, opacity: 0.5, depthWrite: false })
  );
  ...
  for (let i = 0; i < 24; i++) {
    ...
    debris.userData.velocity = new THREE.Vector3(
      (seededRandom(i * 7.1) - 0.5) * 210,
      95 + seededRandom(i * 8.7) * 155,
      (seededRandom(i * 9.3) - 0.5) * 210
    );
