const [coreResponse, overridesResponse, controlUpdatesResponse, cursorAutopilotResponse, qualityFixesResponse, environmentResponse, impactVisualsResponse] = await Promise.all([
  ...
  fetch('./src/impact-visuals.js', { cache: 'no-store' })
]);
...
if (!impactVisualsResponse.ok) throw new Error('Unable to load impact visuals');
...
const impactVisuals = await impactVisualsResponse.text();
